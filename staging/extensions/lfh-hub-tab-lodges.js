...
import {
  ...
  silentVariableUpdate,
  trackHubLodgeExplored, trackHubHighIntent, getVideoEmbedUrl,
...
export function renderLodgesTab(container, config, savedState) {
  const { onSwitchTab, onActionTaken, onCloseHub, requestCloseWithAction, focusLodge, lodgeId, isMobile = false } = config;
  ...
  tabBar.innerHTML = `
    ...
    <button class="lfhlc-tab ${_state.activeSubTab === 'terrain' ? 'active' : ''}" data-tab="terrain">Terrain & Runs</button>
  `;
  ...
  tabBar.querySelectorAll('.lfhlc-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tab.dataset.tab === 'bell2' || tab.dataset.tab === 'ripley') {
        trackHubLodgeExplored(tab.dataset.tab);
      } else if (tab.dataset.tab === 'terrain') {
        trackHubLodgeExplored('terrain');
      }
      updateSubTabState();
      renderActiveSubTab();
    });
  ...
  function renderOverviewTab() {
    ...
    content.querySelectorAll('.lfhlc-view-lodge-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        trackHubLodgeExplored(btn.dataset.lodge);
        renderActiveSubTab();
      });
    });
    ...
    content.querySelectorAll('.lfhlc-overview-card').forEach(card => {
      card.addEventListener('click', (e) => {
        if (!e.target.closest('.lfhlc-view-lodge-btn') && !e.target.closest('.lfhlc-view-lodge-tours-btn')) {
          ...
          trackHubLodgeExplored(card.dataset.lodge);
          renderActiveSubTab();
        }
      });
    });
    ...
    content.querySelectorAll('.lfhlc-view-lodge-tours-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        trackHubHighIntent('view_tours:' + btn.dataset.lodge);
        onSwitchTab('tours', { initialLodgeFilter: btn.dataset.lodge });
      });
    });

    // Help Me Choose — gate behind confirmation
    content.querySelector('.lfhlc-help-choose-btn')?.addEventListener('click', () => {
      trackHubHighIntent('lodge_recommend');
      requestCloseWithAction({ action: 'lodge_recommendation_request', source: 'lodge_compare' });
    });
  ...
  function renderTerrainTab() {
    ...
    content.innerHTML = `
      <div class="lfhlc-terrain-tab">
        ...
        <div class="lfhlc-terrain-grid-2x2">
          <div class="lfhlc-terrain-grid-card" data-action="terrain-lightbox" data-lodge="bell2">
            <img src="${bell2.terrain.aerialContextImage || bell2.terrain.aerialImage}" alt="Bell 2 terrain overview" class="lfhlc-terrain-grid-img" />
            <div class="lfhlc-terrain-grid-label">Bell 2 Terrain</div>
          </div>
          <div class="lfhlc-terrain-grid-card" data-action="terrain-lightbox" data-lodge="ripley">
            <img src="${ripley.terrain.aerialContextImage || ripley.terrain.aerialImage}" alt="Ripley Creek terrain overview" class="lfhlc-terrain-grid-img" />
            <div class="lfhlc-terrain-grid-label">Ripley Creek Terrain</div>
          </div>
          <div class="lfhlc-terrain-grid-card" data-action="layout-lightbox" data-lodge="bell2">
            <img src="${bell2.terrain.locationLayoutImage}" alt="Bell 2 Lodge property layout" class="lfhlc-terrain-grid-img" />
            <div class="lfhlc-terrain-grid-label">Bell 2 Layout</div>
          </div>
          <div class="lfhlc-terrain-grid-card" data-action="layout-lightbox" data-lodge="ripley">
            <img src="${ripley.terrain.locationLayoutImage}" alt="Ripley Creek property layout" class="lfhlc-terrain-grid-img" />
            <div class="lfhlc-terrain-grid-label">Ripley Creek Layout</div>
          </div>
        </div>

        <h3 class="lfhlc-section-title" style="margin-top:24px;">Ski Runs</h3>
        ...
        `}
          <div class="lfhlc-run-photos-group">
            <h4 class="lfhlc-run-photos-heading">Bell 2 Lodge Runs</h4>
            <p class="lfhlc-run-photos-desc">${bell2.terrain.type} — ${bell2.terrain.snowfall}</p>
            <div class="lfhlc-run-photos-strip">${bell2RunPhotosHTML}</div>
          </div>
          <div class="lfhlc-run-photos-group">
            <h4 class="lfhlc-run-photos-heading">Ripley Creek Runs</h4>
            <p class="lfhlc-run-photos-desc">${ripley.terrain.type} — ${ripley.terrain.snowfall}</p>
            <div class="lfhlc-run-photos-strip">${ripleyRunPhotosHTML}</div>
          </div>
        </div>

        <h3 class="lfhlc-section-title" style="margin-top:24px;">Side-by-Side Comparison</h3>
    ...
    // Which Terrain Suits My Skill Level — gate behind confirmation
    content.querySelector('.lfhlc-terrain-recommend-btn')?.addEventListener('click', () => {
      trackHubHighIntent('terrain_recommend');
      requestCloseWithAction({ action: 'terrain_recommendation_request', source: 'lodge_compare' });
    });

    content.querySelectorAll('[data-action="layout-lightbox"]').forEach(card => {
      card.addEventListener('click', () => showLayoutLightbox(card.dataset.lodge));
    });

    content.querySelectorAll('[data-action="terrain-lightbox"]').forEach(card => {
      card.addEventListener('click', () => showTerrainLightbox(card.dataset.lodge));
    });
  ...
  function renderLodgeDetail(lodge) {
        ...
        <div class="lfhlc-detail-scroll">
          <div class="lfhlc-hero-media" id="lfhlc-hero-media">
            <div class="lfhlc-hero-image" style="background-image: url('${lodge.gallery[0]}')">
              ${lodge.videoId ? `<button class="lfhlc-play-btn" id="lfhlc-play-video" data-video-id="${lodge.videoId}">
          ...
          <!-- Maps: Layout + Terrain side by side on desktop -->
          <div class="lfhlc-maps-grid">
            ${lodge.terrain.locationLayoutImage ? `
            <div class="lfhlc-map-card">
              ...
              <div class="lfhlc-map-content open" id="lfhlc-layout-content">
                <img src="${lodge.terrain.locationLayoutImage}" alt="${lodge.name} property layout"
                     class="lfhlc-map-thumb" data-action="layout-lightbox" data-lodge="${lodge.id}" />
                <p class="lfhlc-map-hint">Click to enlarge</p>
              </div>
            </div>
            ` : ''}
            <div class="lfhlc-map-card">
              ...
              <div class="lfhlc-map-content open" id="lfhlc-terrain-content">
                <img src="${lodge.terrain.aerialImage}" alt="${lodge.name} terrain"
                     class="lfhlc-map-thumb" data-action="terrain-lightbox" data-lodge="${lodge.id}" />
                <p class="lfhlc-map-hint">Click to enlarge</p>
              </div>
            </div>
          </div>

          <!-- Terrain Details (Collapsible) -->
          <div class="lfhlc-terrain-section">
            ...
            <div class="lfhlc-terrain-content ${isMobile ? '' : 'open'}" id="lfhlc-terrain-details-content">
              <div class="lfhlc-terrain-details">
                ...
                <div class="lfhlc-terrain-row">
                  <span class="lfhlc-terrain-label">Terrain</span>
                  <div class="lfhlc-feature-tags">${terrainCharsHTML}</div>
                </div>
                <div class="lfhlc-terrain-row">
                  <span class="lfhlc-terrain-label">Bad Weather Skiing</span>
          ...
          <div class="lfhlc-detail-actions">
            <button class="lfhlc-btn-primary lfhlc-action-book" data-lodge="${lodge.id}">View Tours at ${lodge.name}</button>
    ...
    content.querySelector('#lfhlc-play-video')?.addEventListener('click', (e) => {
      playVideo(e.currentTarget.dataset.videoId, lodge);
    });
    ...
    bindToggle('lfhlc-layout-toggle', 'lfhlc-layout-content');
    bindToggle('lfhlc-terrain-toggle', 'lfhlc-terrain-content');
    bindToggle('lfhlc-terrain-details-toggle', 'lfhlc-terrain-details-content');
  ...
  function playVideo(videoId, lodge) {
    const heroMedia = content.querySelector('#lfhlc-hero-media');
    heroMedia.innerHTML = `
...
// ============================================================================
// LIGHTBOX: Terrain Aerial (appended to document.body, z-index 10002)
// ============================================================================
...
function showTerrainLightbox(lodgeId) {
  ...
  const lightbox = document.createElement('div');
  lightbox.className = 'lfhlc-lightbox';
  lightbox.id = 'lfhlc-terrain-lightbox';
  lightbox.style.zIndex = '10002';
  lightbox.innerHTML = `
    <div class="lfhlc-lightbox-backdrop"></div>
    <div class="lfhlc-lightbox-content">
      <button class="lfhlc-lightbox-close">&times;</button>
      <img src="${lodge.terrain.aerialImage}" alt="${lodge.name} Terrain" />
      <p class="lfhlc-lightbox-caption">${lodge.name} Terrain Overview</p>
    </div>
  `;
  document.body.appendChild(lightbox);
  ...
  const escHandler = (e) => {
    if (e.key === 'Escape' && document.getElementById('lfhlc-terrain-lightbox')) closeLightbox();
  };
  document.addEventListener('keydown', escHandler);
}
...
.lfhlc-terrain-grid-label {
  ...
  color: ${LFH_COLORS.textPrimary}; text-transform: uppercase;
  letter-spacing: 0.3px; text-align: center; background: ${LFH_COLORS.infoBox};
}
.lfhlc-terrain-cta { text-align: center; padding: 20px 0 10px; }

/* Run Photos */
.lfhlc-run-photos-section { margin-bottom: 24px; }
.lfhlc-run-photos-group { margin-bottom: 20px; }
.lfhlc-run-photos-group:last-child { margin-bottom: 0; }
.lfhlc-run-photos-heading {
  font-size: 14px; font-weight: 700; color: ${LFH_COLORS.textPrimary};
  margin: 0 0 2px; font-family: 'Inter', sans-serif;
}
.lfhlc-run-photos-desc {
...
.lfhlc-terrain-image { width: 100%; height: auto; border-radius: 8px; }
...
.lfhlc-map-content.open { max-height: 600px; padding: 0 12px 12px; }
.lfhlc-map-thumb {
