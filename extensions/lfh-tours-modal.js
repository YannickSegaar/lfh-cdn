...
export function openTourExplorerModalWithBookingUnified(focusTourId = null, config = {}) {
  ...
  function _teBuildHubActivity() {
    return { toursViewed: viewedTourIds, toursCompared: compareTours, filtersApplied: activeFilters, lastViewed: currentTourId };
  }

  function renderTourDetail(tour) {
    currentView = 'detail';
    currentTourId = tour.id;
    if (viewedTourIds.indexOf(tour.id) === -1) viewedTourIds.push(tour.id);
    filterBar.style.display = '';
  ...
  function openReplaceBooking(tour) {
    ...
    renderBookingForm(formContainer, {
      ...
      conversationId,
      userId,
      hubActivity: _teBuildHubActivity(),
